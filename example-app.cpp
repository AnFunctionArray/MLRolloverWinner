						...
						do {
							...
							if (dobetr) {
								...
								if (betsitesrmade == 400) {
									...
									tolrnll2 = rfgrid.clone().detach();//reswillwino.defined() ? (reswillwino > 0.5).clone().detach().reshape_as(tolrnll2).toType(c10::ScalarType::Float) : tolrnll2;
									rfmsk = (wmsk - (wmsklst)).abs();//( (rfgrid * wmsk) / ((rfgrid - 1.).abs() * wmsklst + 1e-6)).sigmoid();
